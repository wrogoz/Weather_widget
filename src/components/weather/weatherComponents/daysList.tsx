...
export class DaysList extends React.Component<DayListProps, {}> {
  render() {
      ...
      if (key > 0) {
        ...
        return (
          <DaysListElement
            ...
            key={key}
            celcius={
              this.props.store.temperature.length === 0
                ? 0
                : this.props.store.temperature.slice()[key]
            }
            fahrenheit={
              this.props.store.temperature.length === 0
                ? 0
                : Math.floor(
                    (this.props.store.temperature.slice()[key] * 9) / 5
                  ) + 32
            }
            pollen={
              this.props.store.pollenCount.length === 0
                ? 0
                : this.props.store.pollenCount.slice()[key]
            }
            icon={
    ...
    return <ListContainer>{daysListItems}</ListContainer>;
  }
}
